...
function strCount(str, letter){  
  //code here
const lensss = str.split("").length;
const splits = str.split("");
let count = 0;

for(let i = 0; i < lensss; i++){
  if (letter == splits[i]){
    count++
    console.log(`found a match! Count is now ${count}`)
  } else {
    console.log(`at index ${i} no match was found`)
  }
}
return count
}
...
// level 2 = 4
//level 3 =9
// you are given total amount of money = bonus
// price per can i given

// bonus = number of cans * price
// number of cans = number of levels / factorial 
var beeramid = function(bonus, price) {
  let levelCount = 0 ;
  let currentCanCount = 0
  let totalCans = 0
  let grandTotal = 0;
  const perCan = price;
  if (bonus <= 0){
  ...
  while (bonus >= grandTotal) {
    levelCount ++;  // this increments the Level counter
    currentCanCount = Math.pow(levelCount,2); // this counts the cans at the current level 
    totalCans = currentCanCount + totalCans; // this adds the cans to total can count 

    grandTotal = grandTotal + (currentCanCount*perCan);
